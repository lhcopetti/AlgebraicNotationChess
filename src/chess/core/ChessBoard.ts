...
class ChessBoard {
    ...
    public static fromList(localizedChessPieces: [ChessPiece, ChessSquare][]) {
        ...
        for (const piece of localizedChessPieces) {
    ...
    private static emptyBoard(): ChessPiece[][] {
        ...
        for (let i = 0; i < ranks.length; ++i) board[i] = [];
    ...
    public movePiece(origin: ChessSquare, destination: ChessSquare) {
        ...
        if (pieceToMove == null) throw new Error(`No piece to move at: ${origin}`);
    ...
    public getPieces(desiredPiece: ChessPiece): ChessSquare [] {
        ...
        for (let rank = 0; rank < ranks.length; ++rank) {
            for (let file = 0; file < files.length; ++file) {
                ...
                if (pieceFromBoard != null && desiredPiece.equal(pieceFromBoard)) pieces.push(square);
            }
        }
